...
@Component({
  selector: 'SearchArchivedEventsComponent',
  templateUrl: './searcharchivedevents.component.html'
})

export class SearchArchivedEventsComponent implements OnInit {
  ...
  loadData(event: LazyLoadEvent) {
    ...
    console.log(event);
    this.sharedservice.getDataWithFilter(event, this.colVisData, this.maxrecords, this.filters, "searchArchivedEvents", 0)
      .subscribe((resp: any) => {
        // debugger;
  ...
  saveQuery(form) {
    ...
    if (!form.invalid) {
      this.sharedservice.saveQuery('event-archive', null, '', this.saveQueryName, this.filters)
        .subscribe((res: any) => {
          console.log(res);
  ...
  downloadCSV() {
    this.sharedservice.getQueryResultsExport(event, this.colVisData, this.maxrecords, this.filters, "event-archive", null)
      .subscribe((res: any) => {        
        var link = document.createElement('a');
        link.href = window.URL.createObjectURL(res);
        link.download = "Archived-Events.csv";
        link.click();
      })
  }
}
