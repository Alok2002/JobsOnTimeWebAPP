...
import { aggregateBy, process, State } from '@progress/kendo-data-query/dist/es/main';
...
@Component({
  selector: 'MemberPointsDetailComponent',
  templateUrl: './memberpointsdetail.component.html'
})

export class MemberPointsDetailComponent implements OnInit {
  isLoading = false;
  ...
  public state: State = {
    skip: 0,
    take: 25,
    group: [{ field: 'respId', aggregates: this.aggregates }]
  };
  ...
  ngOnInit() {
    ...
    this.reports.jobId = 0;
    //this.getJobReportSummary();
    this.getStaffs();
    this.getReportJobs();
    this.getPointEventTypes();
  }

  getReportJobs() {
    this._reportservie.getPointReportJobs()
      .subscribe((res: any) => {
        console.log(res)
        this.jobList = res.value;
      })
  }

  getPointEventTypes() {
    this._reportservie.getPointEventTypes()
      .subscribe((res: any) => {
        console.log(res)
        this.eventList = res.value;
      })
  }
  ...
  submitFilter() {
    ...
    this._reportservie.postMemberPointsDetailRespondent(this.reports)
      .subscribe((res: any) => {
        console.log(res);
        this.data = res.value;
        this.gridData = process(this.data, this.state);
  ...
  resetFilter() {
    ...
    this.reports.memberStatus = 'Active';
    this.reports.jobId = 0;
    this.cpointstart = null;
    this.cpointend = null;
    this.ppointstart = null;
    this.ppointend = null;
    this.tpointstart = null;
    this.tpointend = null;
  ...
  getNameById(value) {
    var ret = "";
    if (value) {
      var index = this.data.findIndex(dt => dt.respId == value);
      if (index >= 0) ret = this.data[index].firstName + "  " + this.data[index].lastName +
      "&#160;&#160;&#160;|&#160;&#160;&#160;Current: "+ this.data[index].currentPoints +"&#160;&#160;&#160;Paid: "+ this.data[index].paidPoints +"&#160;&#160;&#160;Total: "+ this.data[index].totalPoints;
    }
    return ret;
  }
}