...
@Component({
  selector: 'JobPrivateListComponent',
  templateUrl: './jobprivatelist.component.html',
})

export class JobPrivateListComponent implements OnInit {
  apiHost = apiHost;
  @Input() id: number;
  ...
  constructor(private router: Router, private securityInfoResolve: SecurityInfoResolve,
    ...
    private cookieService: CookieService, private jobservices: JobServices, private clientservices: ClientServices) {
  }
  ...
  loadData() {
    this.jobservices.getJobPrivateList(this.id)
      .subscribe((res: any) => {
        console.log(res)
        this.docs = res.value;
  ...
  submitClientNotes(files) {
    var docData = { 'parentId': this.id, 'parentTableName': 'ClientJob', 'clientContactId': 0, 'username': this.loginnameid };
  ...
  filterSubmit(res) {
    ...
    this.sharedService.getDataWithFilter(null, null, res.maxrecords, this.filters, this.entity, this.id)
      .subscribe((resp: any) => {
