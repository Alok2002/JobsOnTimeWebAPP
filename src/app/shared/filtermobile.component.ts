...
export class FilterMobileComponent implements OnInit {
      ...
      if (this.filterCriterias[this.selectedCriteriaIndex].type == 'DropDown') {
        this.sharedService.getFilterItemList(this.entity, this.filterCriterias[this.selectedCriteriaIndex].caption, this.surveyId)
          .subscribe((res: any) => {
            ...
            this.changeSelectedCriteriaHelper();
          });
      }
      else {
        for (var i = 0; i < this.filterCriCount; i++) {
  ...
  changeSelectedCriteriaHelper() {
    ...
    setTimeout(() => {
      ...
      console.log(this.selectedCriteriaIndex);
    }, 500);
  }
  ...
  getDefaultFilter(resetfilter?) {
    this.sharedService.getDefaultFilter(this.entity, true)
      .subscribe((res: any) => {
        console.log(res);
